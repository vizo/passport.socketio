...
function authorize(options) {
  ...
  return function(data, accept){
    ...
    data.cookie = connectUtils.parseSignedCookies(parsedCookie, auth.session.secret);

    data.sessionID = data.cookie[ auth.session.key ];

    auth.session.store.get(data.sessionID, function(err, session){
      if (err) {
